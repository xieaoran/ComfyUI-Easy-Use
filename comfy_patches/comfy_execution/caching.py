...
class BasicCache:
    ...
    def _clean_cache_recursive(self):
        self.cache.clear()
        for subcache in self.subcaches.values():
            if isinstance(subcache, BasicCache):
                subcache._clean_cache_recursive()
...
class HierarchicalDependencyAwareCache(BasicCache):
    def __init__(self, key_class, name: str = ""):
        super().__init__(key_class)
        self.descendants = {}  # Maps node_id -> set of descendant node_ids
        self.ancestors = {}    # Maps node_id -> set of ancestor node_ids
        self.executed_nodes = set()  # Tracks nodes that have been executed
    ...
    def set(self, node_id, value):
        ...
        self.set_history.append(node_id)

        self.executed_nodes.add(node_id)
        self._cleanup_ancestors(node_id)

    def set_dependency(self, node_id, child_id):
    ...
    async def ensure_subcache_for(self, node_id, children_ids):
        ...
        for child_id in children_ids:
            self.set_dependency(node_id, child_id)

            child_node = self.dynprompt.get_node(child_id)
            if child_node is None:
                raise ValueError(f"Child node {child_id} not found in dynprompt")

            for k, v in child_node["inputs"].items():
                if is_link(v):
                    ancestor_id = v[0]
                    if ancestor_id not in self.descendants:
                        self.descendants[ancestor_id] = set()
                    self.descendants[ancestor_id].add(child_id)
                    self.ancestors[child_id].add(ancestor_id)
    ...
    def _remove_node(self, node_id):
        self.remove_history.append(node_id)

        c = self._get_cache_for(node_id)
        if c is None:
            return
        cache_key = c.cache_key_set.get_data_key(node_id)
        if cache_key in c.cache:
            del c.cache[cache_key]
        subcache_key = c.cache_key_set.get_subcache_key(node_id)
        if subcache_key in c.subcaches:
            c.subcaches[subcache_key]._clean_cache_recursive()
